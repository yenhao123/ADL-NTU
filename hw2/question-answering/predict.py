...
def parse_args():
    ...
    parser.add_argument("--output_path", type=str, default=None, help="Where to store the final result.")
...
def main():
    ...
    test_dataloader = DataLoader(
        predict_dataset_for_model, collate_fn=data_collator, batch_size=args.per_device_test_batch_size
    )
    ...
    predictions = post_processing_function(predict_examples, predict_dataset, outputs_numpy)
    results = {d["id"]: d["prediction_text"] for d in predictions.predictions}

    with open(args.output_path, 'w') as f:
        writer = csv.writer(f)
        writer.writerow(["id","answer"])
        for d in predictions.predictions:
            writer.writerow([d["id"],d["prediction_text"]])

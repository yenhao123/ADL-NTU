...
def preprocess(args,train_df,eval_df,test_df):
    ...
    test_df["course_published_at_local"] = test_df.apply(lambda d:format_to_unix_time(d["course_published_at_local"]),axis=1)
    '''
    ...
    test_item_profile = test_df[item_cols].drop_duplicates('subgroup')
    ...
    x_test = gen_model_input(df_test, test_user_profile, user_col, \
    test_item_profile, item_col, seq_max_len=50)
    y_test = x_test["label"]
    ...
    return user_features, item_features,  x_train, y_train, x_test, all_item

def inference(args, user_features, item_features, x_train, y_train, x_test, all_item):
    
    # 根据之前处理的数据拿到Dataloader
    dg = MatchDataGenerator(x=x_train, y=y_train)

    # 產生 train、test 的 dataloader
    ...
    match_evaluation(args,user_embedding, item_embedding, x_test, all_item, raw_id_maps=raw_id_maps_path, topk=10)


def train(args, user_features, item_features, x_train, y_train, x_test, all_item):
...
def run(args):
    ...
    user_features, item_features, x_train, y_train, x_test, all_item = preprocess(args,train_df,eval_df,test_df)

    inference(args,user_features, item_features, x_train, y_train, x_test, all_item)

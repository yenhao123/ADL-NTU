...
class HahowDataset(Dataset):
    ...
    def __getitem__(self,index):
        d_features = self.dataset[0]
        s_features = self.dataset[1]
        if self.split == "train":
            y = self.dataset[2]

        print(s_features[index])

        return (d_features[index],s_features[index],y[index])
        
    def __len__(self):
        return self.num_batches

def collate_wrapper_hahow_offset(list_of_tuples):
    # where each tuple is (X_int, X_cat, y)
    # If batch_size is 128, 
    # convert ((X_int_1, X_cat_1, y_1),...,(X_int_128, X_cat_128, y_128)) 
    # to ((X_int_1,...,X_int_128),(X_cat_1,...,X_cat_128),(y_1,...y_128))
...
def collate_wrapper_hahow_length(list_of_tuples):
...
# hahow 
def make_hahow_data_and_loader(args,data):
    
    train_data = HahowDataset(
        data,
        args.num_batches,
        split="train"
    )

    test_data = HahowDataset(
        data,
        args.num_batches,
        split="test"
    )

    collate_wrapper_hahow = collate_wrapper_hahow_offset
    if offset_to_length_converter:
        collate_wrapper_hahow = collate_wrapper_hahow_length
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        collate_fn=collate_wrapper_hahow,
        pin_memory=False,

...
def formatToUnixTime(time):
    ...
    for t in time:
        try:
            ...
            timestamps.append(date_time_obj.timestamp())
        except:
            timestamps.append(1594008106.226)

    return timestamps

def doOneHot(df,s_colname):
    print(s_colname)
    s_numpy = df[s_colname].to_numpy().reshape(-1,1)
    onehotencoder = OneHotEncoder()
    s_feature = onehotencoder.fit_transform(s_numpy).toarray()
    return s_feature


def preprocessed(args,df):
    
    print("-----------start preprocess-----------")
    
    d_features,s_features,y = None,[],[]
    ln_emb = []
    ...
    d_feature = np.array(labelencoder.fit_transform(df["user_id"]))
    d_features = d_feature.reshape((d_feature.shape[0],1))

    timestamps = np.array(formatToUnixTime(df["course_published_at_local"]))
    d_feature = timestamps.reshape((timestamps.shape[0],1))
    d_features = np.concatenate((d_features,d_feature),axis=1)
    
    d_feature = np.array(df["course_price"])
    d_feature = d_feature.reshape((d_feature.shape[0],1))
    d_features = np.concatenate((d_features,d_feature),axis=1)
    ...
    for i,s_colname in enumerate(s_colnames):
        # s_feature type numpy array
        s_feature = doOneHot(df,s_colname)
        ln_emb.append(len(s_feature[0]))
        if i == 0:
    ...
    # label
    y.append(df["course_id"].to_numpy().tolist())

    return [d_features,s_features,y],ln_emb

def summaryCSV(args):
    inputDir = args.inputDir
    # splits course, number of columns: 59737 -> 139608
    trainProcessed = {"user_id":[],"course_id":[]}
    train = pd.read_csv(inputDir+"/train.csv")

    for i,cids in enumerate(train["course_id"]):
        ids = cids.split(" ")
        for idx in ids: 
            trainProcessed["user_id"].append(train["user_id"][i])
            trainProcessed["course_id"].append(idx)

    # trainDf = pd.DataFrame(trainProcessed)

    # add course features
    course = pd.read_csv(inputDir+"/courses.csv")
    for col in course.columns:
        if col != "course_id":
            trainProcessed.update({col:[]})

    for cid in trainProcessed["course_id"]:
        mask = course["course_id"].str.match(pat = cid)
        selectedRows = course[mask].drop(columns=["course_id"])

        for col in selectedRows.columns:
            trainProcessed[col].append(selectedRows[col].values[0])
    
    trainDf = pd.DataFrame(trainProcessed)
    trainDf.to_csv(inputDir + "/trainSummary.csv",index=False)

    # add chapter features
    #chapter = pd.read_csv(inputDir+"/course_chapter_items.csv")
    return trainDf
...
def run():
    ...
    if args.data_generation == "dataset":
        ...
        ln_bot[0] = m_den
    elif args.data_generation == "hahow":
        # summary course.csv、chapter.csv、train.csv info 
        ...
        # data process, e.g, 
        # * fill NAN data
        # * split dense_features、sparse_features、label
        dataset,ln_emb = preprocessed(args,df)

        train_data, train_ld, test_data, test_ld = dp.make_hahow_data_and_loader(args,dataset)  
        
        ln_emb = np.array(ln_emb)
        # number if dense features
        m_den = 3
        predSize = 728
        ln_bot[0] = m_den
    else:
    ...
    print("ln_bot" + str(ln_bot))
    print("ln_top" + str(ln_top))
    global dlrm
    dlrm = DLRM_Net(
        m_spa,
        ln_emb,
